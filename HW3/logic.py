...
f2 = f = fetch_rules_for_goal(kb1, 'HasSymptom(x,Diarrhea)')
...
theta2 = {}
kb2 = {'Rules': [Rule(lhs=['American(x)', 'Weapon(y)', 'Sells(x,y,z)', 'Hostile(z)'], rhs='Criminal(x)'), Rule(lhs=['Missile(x)', 'Owns(Nono,x)'], rhs='Sells(West,x,Nono)'), Rule(lhs=['Missile(x)'], rhs='Weapon(x)'), Rule(lhs=['Enemy(x,America)'], rhs='Hostile(x)')], 'Facts': ['American(West)', 'Enemy(Nono,America)', 'Owns(Nono,M)', 'Missile(M)']}
